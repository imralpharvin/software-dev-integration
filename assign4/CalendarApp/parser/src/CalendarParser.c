...
char* eventListToJSONv2(const List* eventList)
{
    ...
    void* elem = nextElement(&iter);
    Event * theEvent = (Event*) elem;
    char * jEvent = eventToJSONv2(theEvent);
    jString = realloc(jString, strlen(jString) + strlen(jEvent) + 50 );
    strcat(jString, jEvent );
    free(jEvent);
...
char* eventToJSONv2(const Event* event)
{
  char * jString;
  char * jEvent = eventToJSON(event);

  jEvent[strlen(jEvent)-1] = '\0';


  char * alarmList = alarmListToJSON(event->alarms);
  char * propertyList = propertyListToJSON(event->properties);
  jString = malloc(strlen(jEvent) + strlen(alarmList)+ strlen(propertyList) +50);
  sprintf(jString , "%s,\"alarmList\":%s,\"properties\":%s}", jEvent, alarmList, propertyList);
  return jString;
}
...
char * filenameToJSON(char * filename)
{
  ...
  ICalErrorCode err = createCalendar(filename, &newCalendar);

  if(err != OK)
  {
    return "{}";
  }
  char * string = calendarToJSON(newCalendar);

  return string;
}

char* filenameToEventListJSON(char * filename)
{
  Calendar * newCalendar;
  ...
  char * string = eventListToJSONv2(newCalendar->events);
...
char * alarmToJSON(const Alarm * theAlarm)
{
  if(theAlarm == NULL){
    return "{}";
  }
  ...
  ListIterator iter = createIterator(theAlarm->properties);
  void* elem;
  while((elem = nextElement(&iter)) != NULL){
    numProps ++;
  }

  char * jString = malloc(strlen(theAlarm->action) + strlen(theAlarm->trigger) + sizeof(char) *250);

  sprintf(jString, "{\"action\":\"%s\",\"trigger\":\"%s\",\"numProps\":%d}", theAlarm->action, theAlarm->trigger, numProps );
...
{
  if((List *)alarmList == NULL || getLength((List *)alarmList) == 0){
    return "[]";
  }
  char * jString = malloc(sizeof(char) *100);
  strcpy(jString, "[");
  ...
  void* elem = nextElement(&iter);
  Alarm * theAlarm = (Alarm*) elem;
  char * jAlarm = alarmToJSON(theAlarm);
  jString = realloc(jString, strlen(jString) + strlen(jAlarm) + 50 );
  strcat(jString, jAlarm );
  free(jAlarm);

  while((elem = nextElement(&iter)) != NULL){
    Alarm * theAlarm = (Alarm*) elem;
    strcat(jString, ",");
    jAlarm = alarmToJSON(theAlarm);
    jString = realloc(jString, strlen(jString) + strlen(jAlarm) + 50 );
    strcat(jString, jAlarm );
    free(jAlarm);
  }
...
char * propertyToJSON(const Property* property)
{
  ...
  char * jString = malloc(strlen(property->propName) + strlen(property->propDescr) + 50);

  sprintf(jString, "\"%s\":\"%s\"", property->propName, property->propDescr );
...
{
  if(getLength((List*)propertyList) == 0 || (List*)propertyList == NULL)
  {
    return "{}";
  }
  char * jString = malloc(sizeof(char) + 1);
  strcpy(jString, "");
  ListIterator iter = createIterator( (List *)propertyList);
  void* elem = nextElement(&iter);
  Property * theProperty = (Property*) elem;
  char * jProperty = propertyToJSON(theProperty);
  jString = realloc(jString, strlen(jString) + strlen(jProperty) + 50 );
  strcat(jString, "{");
  strcat(jString, jProperty );
  free(jProperty);

  while((elem = nextElement(&iter)) != NULL){
    Property * theProperty = (Property*) elem;
    strcat(jString, ",");
    jProperty = propertyToJSON(theProperty);
    jString = realloc(jString, strlen(jString) + strlen(jProperty) + 50 );
    strcat(jString, jProperty );
    free(jProperty);
  }
...
void createAndWriteCalendar(char * filename, char * str, char * eventStr, char * dtstartStr, char * dtstampStr)
{

 Calendar * theCalendar = JSONtoCalendar(str);
 Event * theEvent = JSONtoEvent(eventStr);
 DateTime dtStart = JSONToDT(dtstartStr);
 DateTime dtStamp = JSONToDT(dtstampStr);
 theEvent->creationDateTime = dtStamp;
 theEvent->startDateTime = dtStart;


 addEvent(theCalendar, theEvent);

 writeCalendar(filename, theCalendar);

}


void addEventAndWriteCalendar(char * filename, char * eventStr, char * dtstartStr, char * dtstampStr)
{

 Calendar * theCalendar;
 createCalendar(filename, &theCalendar);
 Event * theEvent = JSONtoEvent(eventStr);
 DateTime dtStart = JSONToDT(dtstartStr);
 DateTime dtStamp = JSONToDT(dtstampStr);
 theEvent->creationDateTime = dtStamp;
...
void addEventandWriteCalendar(char * filename, char * str)
{
/*
 Event * theEvent = JSONtoEvent(str);

 ICalErrorCode err = writeCalendar(filename, theCalendar);*/

}

List * JSONtoEventList(const char* str)
{
...
DateTime JSONToDT(char * str)
{
  ...
  printf("1: %s\n", token);
  strcpy(theDT.date, token);
  token = strtok(NULL, ":");

  token = strtok(NULL, "\"");
  printf("2: %s\n", token);
  strcpy(theDT.time, token);
  token = strtok(NULL, ":");
  token = strtok(NULL, "}");
  printf("3: %s\n", token);
